	...
	<form name="filesForm" class="section" action="#" method="post">
		...
		<input type="text" name='maxUploadSizeValue' id="maxUploadSizeValue"
			<?php if (!$_['uploadChangable']): ?>
				value="<?php p($_['uploadMaxFileSizeValue'] . ' ' . $_['maxFileSizeUnits'][$_['uploadMaxFileSizeUnit']]) ?>" disabled="disabled"
			<?php else: ?>
		...
		<select name="maxUploadSizeUnit" id="maxUploadSizeUnit" <?php if(!$_['uploadChangable']) { p('disabled'); } ?>>
			<?php foreach ($_['maxFileSizeUnits'] as $unit => $translatedUnit):
				$selected = '';
				if ($unit == $_['uploadMaxFileSizeUnit']):
					$selected = ' selected="selected"';
				endif;
			?>
				<option value="<?php p($unit)?>" <?php p($selected) ?>><?php p($translatedUnit) ?></option>
			<?php endforeach;?>
		</select>
		<?php endif;?>
