...
abstract class OC_Connector_Sabre_Node implements Sabre_DAV_INode, Sabre_DAV_IProperties {
	 ...
	 */
	public function __construct($info, $view) {
		$this->info = $info;
		$this->view = $view;
		$this->path = $this->view->getRelativePath($info->getPath());
	}
	...
	public function setName($name) {
		...
		$this->view->rename($this->path, $newPath);
		...
		foreach ($properties as $propertyName => $propertyValue) {
			...
			if (is_null($propertyValue)) {
				...
				if (strcmp($propertyName, self::GETETAG_PROPERTYNAME) === 0) {
					$this->view->putFileInfo($this->path, array('etag' => $propertyValue));
