...
class Root extends Folder implements Emitter {
	 ...
	 */
	public function __construct($manager, $user, $userManager = null) {
		parent::__construct($this, null, '', '', array());
	...
	/**
	 * Get the home folder for the user for which the filesystem is setup
	 *
	 * @return \OC\Files\Node\Folder
	 */
	public function getUserFolder() {
		return $this->get('/' . $this->user->getUID() . '/files');
	}
	...
	public function get($path) {
		...
		if ($this->isValidPath($path)) {
			...
			if ($storage->file_exists($internalPath)) {
				$node = $this->createNode($storage, $internalPath, $path);
				$data = $node->getFileInfo();
				return $this->createNode($storage, $internalPath, $path, $data);
			} else {
